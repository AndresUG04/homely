    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3 mt-4">
        ...
        <div className="flex items-center gap-1.5 text-[#5C3A1E]/60">
          ...
          <span className="text-xs truncate max-w-[150px]">{formatSchedule(job.schedule)}</span>
        </div>
      ...
      >
        <div className="p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={onClose}
              className="text-[#5C3A1E]/40 hover:text-[#5C3A1E]/60 text-2xl leading-none"
            >
              &times;
            </button>
          </div>
          ...
          <div className="mt-6 space-y-5">
            <div>
              <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Horario</p>
              <p className="text-base text-[#2C1A0E] mt-1">{formatSchedule(job.schedule)}</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Ubicación</p>
            ...
            <div>
              <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Salario</p>
              <p className="text-lg font-bold text-[#D06224] mt-1">{salary}</p>
            </div>

            {job.employer?.user && (
              <div>
                <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Empleador</p>
                <div className="flex items-center gap-1.5 text-base text-[#2C1A0E] mt-1">
                  <User className="w-4 h-4" />
            ...
            <div>
              <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Descripción</p>
              <p className="text-sm text-[#2C1A0E] mt-1 whitespace-pre-wrap">{job.description || "No disponible"}</p>
            </div>

            {tasks.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-[#D06224] uppercase tracking-wide">Tareas</p>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#5C3A1E]/60 mt-1">
          Explorá las ofertas disponibles
        </p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
          ...
          <p className="text-sm text-[#5C3A1E]/60 font-medium">Cargando ofertas...</p>
        </div>
      ) : error ? (
        <div className="flex flex-col items-center justify-center h-64 gap-3">
